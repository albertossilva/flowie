...
Then('the result is {string}', async function (this: FlowieTestsWorld, result: string) {
  assert.equal(this.flowieResult.lastResults, result, 'Something went wrong and the result does not match')
})
...
Then('the final result will match', function (this: FlowieTestsWorld, resultDocString: string) {
  ...
  assert.deepEqual(this.flowieResult.lastResults, expectedResult.result)
})
