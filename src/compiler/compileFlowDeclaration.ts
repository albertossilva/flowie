...
export default function compileFlowDeclaration<Argument, Result, Context>(
  ...
  const { generatedFlowFunction, generatedFlowFunctionWithContext } = generateFunctionFromFlowDeclaration<
    Argument,
    Result,
    Context
...
function concatenateReportListAndResult(
  ...
  return [functionReportList.concat(functionReport), resultList.concat(result)]
}
