...
function PromiseQueue<Argument, Context, Result>(
  ...
  return {
    async enqueue(...argumentsList: ReadonlyArray<any>) {
      return new EnqueuedPromises<Argument, Result, Context>(callback, parallelPromises, [], 0).enqueue(
        ...argumentsList,
      )
    },
...
class EnqueuedPromises<Argument, Result, Context> implements Queue<Argument, Result, Context> {
  ...
  private async enqueueOrAwaitPromisesToBeResolved(
    ...
    return promiseEnqueuedList
      .slice(0, indexOnList)
      .concat(promiseEnqueuedList.slice(indexOnList + 1), this.createPromiseOnQueue(argumentsList))
  }
...
async function getRacingPromises<Result>({ promise, index }: PromiseOnQueue<Result>): Promise<number> {
  await promise
  return index
}
