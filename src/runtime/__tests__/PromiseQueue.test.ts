...
describe('runtime/PromiseQueue', function () {
  it('returns the last promise, and process them in parallel', async function () {
    const stubFunction = stub()
      .named('stubbed')
      .onFirstCall()
      .callsFake(returnAfter({ time: 7, value: 'not-the-last' }))
      .onSecondCall()
      .callsFake(returnAfter({ time: 5, value: expected }))
  ...
  it('does not add on the promise the last promise, and process them in parallel', async function () {
    const stubFunction = stub()
      .named('stubbed')
      .onFirstCall()
      .callsFake(returnAfter({ time: 30, value: 'not-the-last' }))
      .onSecondCall()
      .callsFake(returnAfter({ time: 5, value: 'not-the-last' }))
      .onThirdCall()
      .callsFake(returnAfter({ time: 1, value: expected }))
