...
describe('runtime.createFlowie (integration tests as laboratory)', function () {
  ...
  context('functions', function () {
    ...
    describe('split', function () {
      ...
      it('executes functions in parallel, but later functions does not affect the order of the results', async function () {
  ...
  describe('generators/iterators', function () {
    ...
    it('returns the last item of generator when it is the last item', function () {
      ...
      const flow = createFlowie(createByPassFunction()).pipe(createByPassFunction()).pipe(generatorMock)
  ...
  describe('context', function () {
    it('sends the context of same type on every flowItem', async function () {
      ...
      const commonFunction = createSimpleFunctionMock(parameter, expected) as (
        argument: string,
        context: string,
      ) => string

      const asyncFunction = createAsyncFunctionMock(expected, 'Split') as (
        argument: string,
        context: string,
      ) => Promise<string>
      ...
      const subFlowFunction = createSimpleFunctionMock(expected, 'subFlow') as (
        argument: string,
        context: string,
      ) => string
      ...
      const flow = createFlowie(commonFunction).split(
        asyncFunction,
        generatorFunction,
        commonFunctionWithoutContext,
        subFlow,
      )
...
const createPrefixed = (prefix: string) => {
  ...
  return stub()
    .named(`prefixed_${prefixForFunctionName}`)
...
function createSimpleFunctionMock(
  parameter: string,
  result: string,
  { functionName = lorem.word(), timesToBeExecuted = 1 } = {},
...
function createAsyncFunctionMock(
  ...
  const asyncMock = mock().withArgs(parameter).exactly(timesToBeExecuted).resolves(result)
